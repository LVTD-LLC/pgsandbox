...
canonicalUrl: "https://pgsandbox.dev/blog/postgres-explain-plan-agent-sql/"
heroImageUrl: ""
featured: false
sortOrder: 100
---
...
6. Run [`run_sql` with bounded Postgres results](https://pgsandbox.dev/blog/postgres-run-sql-bounded-results/) using `readonly: true` and a small `rowLimit` for read proof.
...
PGSandbox's `explain_query` tool uses that shape directly. The [MCP tool contract](https://pgsandbox.dev/docs/mcp-tools/) documents `explain_query` as returning `EXPLAIN (FORMAT JSON)` for one safe plannable statement, plus a compact summary of node types, relations, cost, and estimated rows. It does not use `ANALYZE`, rejects multi-statement SQL, and rejects transaction or session controls.
...
The [Postgres test database guide](https://pgsandbox.dev/blog/how-to-create-postgres-test-database-agent-sql/) covers the broader proof loop for generated SQL. EXPLAIN belongs near the front of that loop, before the agent treats execution output as validation.
...
Create a [database sandbox](https://pgsandbox.dev/blog/what-is-database-sandbox/) first. The sandbox should have the schema shape the query needs, the smallest useful fixture data, a scoped role, a TTL, and a cleanup path. If realistic schema or approved sample data matters, clone or seed it into the sandbox instead of pointing the agent at the source database.

PGSandbox's current resource model is one database, one scoped login role, TTL metadata, and cleanup tied to tracked resources. The [architecture docs](https://pgsandbox.dev/docs/architecture/) describe that model, and the [MCP tool docs](https://pgsandbox.dev/docs/mcp-tools/) document the lifecycle tools around it.
...
After the plan passes review, run a [bounded proof query](https://pgsandbox.dev/blog/postgres-run-sql-bounded-results/).

PGSandbox's `run_sql` executes through sandbox role credentials, not the admin connection. Its docs define a default `rowLimit` of 100, allow `rowLimit: 0` for a zero-row preview, cap returned rows at 1000, and return ordered per-statement result sets for multi-statement SQL (https://pgsandbox.dev/docs/mcp-tools/).
...
The [database migration testing workflow](https://pgsandbox.dev/blog/database-migration-testing-agent-pr/) covers that PR gate. The [schema snapshot guide](https://pgsandbox.dev/blog/postgres-schema-snapshots-agent-migration-reviews/) goes deeper on named before/after checkpoints and compact diffs.
