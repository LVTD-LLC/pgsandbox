...
canonicalUrl: "https://pgsandbox.dev/blog/how-to-clone-postgres-database-sandbox/"
heroImageUrl: ""
featured: false
sortOrder: 40
---
...
The result is not a database branch. It is a [database sandbox](https://pgsandbox.dev/blog/what-is-database-sandbox/) with a copy of the source shape the agent can safely inspect, mutate, and throw away.
...
PGSandbox implements that boundary through the [`clone_database` MCP tool](https://pgsandbox.dev/docs/mcp-tools/). The tool creates an isolated sandbox first, then restores the source into that sandbox with PostgreSQL client tools. The destination still follows the normal [PGSandbox resource model](https://pgsandbox.dev/docs/architecture/): one database, one login role, scoped credentials, TTL metadata, and cleanup tied to resources PGSandbox created.
...
PGSandbox's docs warn not to paste production URLs into prompts when a secret input or local environment variable can provide them (https://pgsandbox.dev/docs/mcp-tools/). Keep that rule. A source connection string is a credential, not documentation.
...
In PGSandbox, the admin connection performs lifecycle work. Tool calls that run user SQL connect with the generated sandbox role. The [architecture notes](https://pgsandbox.dev/docs/architecture/) call this out directly because it is the safety boundary: lifecycle authority and task SQL are different jobs.
...
PGSandbox exposes `describe_schema` for structured table, column, index, and extension summaries, plus `run_sql` with a capped `rowLimit` for validation queries (https://pgsandbox.dev/docs/mcp-tools/). That is usually enough for an agent to confirm it has the right shape without dumping full tables into context.
...
PGSandbox's `clone_database` notes say that if restore fails, PGSandbox attempts to delete the newly created sandbox (https://pgsandbox.dev/docs/mcp-tools/). The broader cleanup model is metadata-backed: `delete_database` deletes a database and role created by PGSandbox, while `cleanup_expired` deletes expired resources with dry-run support.
...
If you are setting this up for the first time, start with the [PGSandbox install guide](https://pgsandbox.dev/docs/install/) and the [Postgres MCP server safety checklist](https://pgsandbox.dev/blog/postgres-mcp-server-safety-checklist/). The clone workflow is strongest when it sits inside the broader safety model: narrow tools, scoped credentials, bounded query output, and cleanup you can audit.
