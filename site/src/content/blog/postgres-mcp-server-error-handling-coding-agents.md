...
canonicalUrl: "https://pgsandbox.dev/blog/postgres-mcp-server-error-handling-coding-agents/"
heroImageUrl: ""
featured: false
sortOrder: 120
---
...
The [MCP tool contract](https://pgsandbox.dev/docs/mcp-tools/) documents the shared fields:
...
2. If it is `undefined_column` or `undefined_table`, call [`describe_schema`](https://pgsandbox.dev/docs/mcp-tools/).
3. Fix identifiers, schema qualification, or `search_path`.
4. If the query is non-trivial, inspect the plan with [`explain_query`](https://pgsandbox.dev/blog/postgres-explain-plan-agent-sql/).
5. Run bounded proof with [`run_sql`](https://pgsandbox.dev/blog/postgres-run-sql-bounded-results/).
...
For agents deciding which selector to use and when to pin a fixed profile, use [How to Use Local Postgres Versions with Coding Agents](https://pgsandbox.dev/blog/how-to-use-local-postgres-versions-with-coding-agents/).
