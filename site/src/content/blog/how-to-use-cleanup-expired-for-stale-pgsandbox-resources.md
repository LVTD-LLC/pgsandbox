...
canonicalUrl: "https://pgsandbox.dev/blog/how-to-use-cleanup-expired-for-stale-pgsandbox-resources/"
heroImageUrl: ""
featured: false
sortOrder: 131
---
...
The official contract for `cleanup_expired` is explicit: use `dryRun` to preview matches, then run without `dryRun` to delete those expired resources (https://pgsandbox.dev/docs/mcp-tools/).
...
The architecture notes and cleanup section say cleanup should only delete databases listed in metadata and matching the configured prefix (https://pgsandbox.dev/docs/architecture/).
...
`cleanup_expired` also supports owner and label filters, which keeps cleanup actionable in shared environments where multiple agents or workflows share the same Postgres profile (https://pgsandbox.dev/docs/mcp-tools/).
...
The agent workflow docs show the same pattern: list by owner first, then call `cleanup_expired` with `includeAllVersions: true` as a separate step when you need the broad view (https://pgsandbox.dev/docs/agent-workflows/).
...
- [PGSandbox MCP Tool Contract](https://pgsandbox.dev/docs/mcp-tools/) for all cleanup inputs and output fields.
- [PGSandbox Architecture Notes](https://pgsandbox.dev/docs/architecture/) for metadata and resource lifecycle details.
- [PGSandbox Agent Workflows](https://pgsandbox.dev/docs/agent-workflows/) for practical cleanup recipes.
