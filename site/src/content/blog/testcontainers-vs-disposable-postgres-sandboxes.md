...
canonicalUrl: "https://pgsandbox.dev/blog/testcontainers-vs-disposable-postgres-sandboxes/"
heroImageUrl: ""
featured: false
sortOrder: 70
---
...
PGSandbox wraps that lifecycle in a narrow MCP tool surface. The [MCP tool contract](https://pgsandbox.dev/docs/mcp-tools/) exposes database lifecycle actions such as creating, cloning, listing, deleting, cleaning up, running bounded SQL, and describing schema. The [architecture docs](https://pgsandbox.dev/docs/architecture/) describe the local-first resource model: one sandbox database, one scoped login role, TTL metadata, and cleanup tied to resources PGSandbox created.
...
For migration-heavy agent work, the [database migration testing workflow](https://pgsandbox.dev/blog/database-migration-testing-agent-pr/) is the more specific version of this pattern: create a sandbox, run the repo migration command, capture the schema diff, test risky data rows, and put the proof in the PR.

That is the workflow behind the [Postgres test database guide](https://pgsandbox.dev/blog/how-to-create-postgres-test-database-agent-sql/). Create a task database, apply the needed schema, seed or clone the smallest useful state, run generated SQL with bounded output, record the result, and delete the database and role.

PGSandbox makes that loop available to MCP clients without requiring the agent to start Docker, bind `localhost:5432`, or handle raw admin credentials. The [install guide](https://pgsandbox.dev/docs/install/) covers the local-first setup and explicitly keeps existing Docker or developer Postgres services on port `5432` out of the way.
