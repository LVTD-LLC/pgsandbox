...
canonicalUrl: "https://pgsandbox.dev/blog/what-is-database-sandbox/"
heroImageUrl: ""
featured: false
sortOrder: 50
---
...
PGSandbox uses this contract as the product shape. One task gets one database, one scoped login role, TTL metadata, labels, bounded SQL results, and cleanup tied to resources PGSandbox created. The [architecture docs](https://pgsandbox.dev/docs/architecture/) describe that resource model in more detail.
...
The [PGSandbox tool contract](https://pgsandbox.dev/docs/mcp-tools/) is intentionally small: create a sandbox, clone a source into a sandbox, run bounded SQL, describe schema, list tracked sandboxes, delete a tracked sandbox, and clean up expired resources. That does not make an agent incapable of bad SQL. It gives the bad SQL a smaller place to land.

When the task is specifically about generated SQL or migrations, the practical workflow is a [Postgres test database for agent-generated SQL](https://pgsandbox.dev/blog/how-to-create-postgres-test-database-agent-sql/): create a task database, apply the right schema state, run the agent SQL through a scoped role, capture bounded proof, and clean up the resource.
...
That is why the [Postgres clone database sandbox guide](https://pgsandbox.dev/blog/how-to-clone-postgres-database-sandbox/) recommends a one-way clone path: read from the source, restore into a newly created disposable destination, run task SQL only against the sandbox role, and clean up the destination afterward.
...
If you are already reviewing database access for agents, start with the [Postgres MCP server safety checklist](https://pgsandbox.dev/blog/postgres-mcp-server-safety-checklist/). If you want to wire the local tool into Codex, Cursor, VS Code, or Claude Desktop, the [install guide](https://pgsandbox.dev/docs/install/) covers the setup path.
